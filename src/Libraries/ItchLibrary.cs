...
internal class ItchLibrary : StoreLibrary<ItchGame, ItchHandler>
{
    ...
    protected override ItchHandler GetHandler()
    {
        Console.WriteLine("Initializing Itch Handler...");
        ...
        var handler = new ItchHandler(fileSystem, registry);

        Console.WriteLine("Itch Handler Initialized!");

        return handler;
    }

    public override ItchGame? Get(object id)
    {
        var gameid = ItchGameId.From(id.ToString());
    ...
    public override ItchGame[] GetAll()
    {
