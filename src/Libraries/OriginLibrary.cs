...
internal class OriginLibrary : StoreLibrary<OriginGame, OriginHandler>
{
    ...
    protected override OriginHandler GetHandler()
    {
        Console.WriteLine("Initializing Origin Handler...");
        ...
        var handler = new OriginHandler(fileSystem, registry);

        Console.WriteLine("Origin Handler Initialized!");

        return handler;
    }

    public override OriginGame? Get(object id)
    {
        var gameid = OriginGameId.From(id.ToString());
    ...
    public override OriginGame[] GetAll()
    {
