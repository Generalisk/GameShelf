...
internal class RockstarLibrary : StoreLibrary<RockstarGame, RockstarHandler>
{
    ...
    protected override RockstarHandler GetHandler()
    {
        Console.WriteLine("Initializing Rockstar Handler...");
        ...
        Console.WriteLine("Rockstar Handler Initialized!");

        return handler;
    }

    public override RockstarGame? Get(object id)
    {
        var gameid = RockstarGameId.From(id.ToString());
    ...
    public override RockstarGame[] GetAll()
    {
        var games = new List<RockstarGame>();
